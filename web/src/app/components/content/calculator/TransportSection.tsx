    ...
    <FormSection legendText="Transport" index={3}>
      <StTransportInputs>
        ...
        <StTransportInput style={{ flexDirection: 'row', display: 'flex' }}>
          <div style={{ width: '50%' }}>
            <StandardDatePicker
              id="transportFrom"
              onChange={(e: any) => {
                props.setTransport({ ...props.transport, fromDate: e });
              }}
              value={
                isFilled(props.transport.fromDate)
                  ? new Date(props.transport.fromDate)
                  : undefined
              }
              required={true}
              label={'From'}
          ...
          <div style={{ width: '50%' }}>
            <StandardDatePicker
              id="transportTo"
              onChange={(e: any) => {
                props.setTransport({ ...props.transport, toDate: e });
              }}
              value={
                isFilled(props.transport.toDate)
                  ? new Date(props.transport.toDate)
                  : undefined
              }
        ...
        {isFilled(props.transport.fromDate) &&
          isFilled(props.transport.toDate) && (
            <StDateInfo>
              {props.transport.fromDate < props.transport.toDate &&
                daysDiff &&
                hoursDiff && (
                  <span>
                    <b>{daysDiff}</b> days and <b>{hoursDiff}</b> hours
                  </span>
                )}
            </StDateInfo>
          )}
