...
import {
  agingHeaders,
  fluidHeaders,
} from '../../../common/tableHeaders/CalculationHeaders';
...
export const CalculationContainer: React.FunctionComponent<ICalculationContainer> = props => {
  ...
  const [resultType, setResultType] = useState<
    'ShipAgeing' | 'GasComposition' | 'LiquidComposition'
  >('ShipAgeing');

  const initRow: IResult = props.calculation.result[0];
  const initRowFluid: IFluid = props.calculation.result[0].gas;
  const endRow: IResult =
    props.calculation.result[props.calculation.result.length - 1];
  ...
  return (
    <StCalculationContainer style={props.style}>
      <H2 style={{ width: '100%' }}>
        ...
        >
          {moment(props.calculation.createdDate).fromNow()}
        </span>
      </H2>
        ...
        {cardResults.map((result: ICardResult, index) => {
          return (
            <StCalculation key={index}>
              <span style={{ fontSize: '18px', margin: '20px 0 10px 0' }}>
              ...
              >
                {result.initValue.toFixed(result.fixed)}
              </span>
              <span
                style={{
                  color: EColor.GREEN,
              ...
              >
                {result.endValue.toFixed(result.fixed)}
              </span>
            </StCalculation>
          );
        })}
        ...
        <>
          <StMoreResults>
            ...
            {resultType === 'ShipAgeing' && (
              <p>
                ISO6976 (2016) are used for calculating WI and GCV. The LNG
                liquid density is calculated based on ISO6578.
              </p>
            )}
            {resultType === 'GasComposition' && (
              <p>
                Composition of boil-off gas is calculated based on SRK-EoS with
                classic mixing rule.
              </p>
            )}
            {resultType === 'LiquidComposition' && (
              <p>
                Liquid is assumed to be at the boiling point. Bubble point
                temperature is calculated based on SRK-EoS with classic mixing
                rule.
              </p>
            )}
          </StMoreResults>
